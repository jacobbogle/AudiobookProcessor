...
def test_part_titles_mutate_and_convert(tmp_path):
    ...
    # Fallback: if the deterministic copy used a different temp name, search the mutated_path directly
    if not files:
        import glob
    ...
    for idx, fp in enumerate(files, 1):
        ...
        try:
            ...
            # If reading fails, try mutagen.mp3 wrapper
            audio = MP3(str(fp))
            # MP3 wrapper may expose tags differently; fall back to filename check
            value = None
    ...
    # Inspect produced M4B for chapter titles
    from mutagen.mp4 import MP4
    m = MP4(out_m4b)
    chapters = getattr(m, 'chapters', None)
    assert chapters and len(chapters) >= count

    # Check first few chapter titles. Chapter atoms should be minimal/unpadded
    # (i.e., "Part {part} - {index}") even though per-file tags include the
    # cleaned folder prefix.
